...
import { dimensionsApi } from '@/api/master'
...
export function DimensionsPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; item?: Dimension }>({ open: false })
  ...
  const { mutate: save, isPending: saving } = useMutation({
    mutationFn: (d: FormData) =>
      modal.item ? dimensionsApi.update(modal.item.id, d) : dimensionsApi.create(d),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['dimensions'] })
      toast.success(modal.item ? 'Dimension updated' : 'Dimension created')
      setModal({ open: false })
    },
  ...
  const { mutate: remove, isPending: deleting } = useMutation({
    mutationFn: (id: string) => dimensionsApi.delete(id),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['dimensions'] })
  ...
  function openEdit(item: Dimension) {
  ...
  return (
    <div>
      <PageHeader
        title="Dimensions"
        subtitle="Manage measurement dimensions (Length, Weight, Volume, etc.)"
        action={
      ...
      <Modal
        ...
        title={modal.item ? 'Edit Dimension' : 'Add Dimension'}
        footer={
      ...
      >
        <form className="flex flex-col gap-4">
          ...
          <Input label="Code" required maxLength={FL.codeUom} placeholder="e.g. gm, kg, ltr" error={errors.code?.message} {...register('code')} />
        </form>
      </Modal>
